    ...
    <nav className="h-14 sm:h-[3.7rem] w-full lg:h-14 bg-kryptonite z-40 flex justify-between sticky top-0 text-white sm:px-0.5 lg:px-4">
      ...
      <div className="flex sm:h-full lg:w-1/2 w-full lg:justify-between justify-end">
          ...
          <div className="aspect-square flex h-full ">
            <button className='flex-1 max-lg:hidden relative' onClick={()=>{popupClasses.map((x)=>{document.getElementById("profile-pop-up").classList.toggle(x)})}}><PiUserCircleFill className='h-full w-full text-[16px] sm:p-2.5'/>
            </button>
      ...
      <section id="profile-pop-up" className={`absolute !min-w-0 !max-w-0 h-0 !opacity-0 h-[43.3vh] duration-300 opacity-100 !aspect-[2/2.4] w-[19vw] max-w-[280px] min-w-[220px] overflow-hidden bg-white shadow-[0px_4px_13px_-2px_black] top-[92%] rounded-2xl right-[2.1%] flex items-center border-2 border-lime-600 outline outline-white`}>
