  ...
  const [isShow, setIsShow] = useState(false)
   ...
   if (type == "password") {
      ...
      <div className={`flex relative items-center max-lg:items-start max-sm:items-center border-b-[1.7px] border-[#fff]`}>
         <input type={'password'} placeholder=' ' autoComplete='on' className={`sm:text-[20px] peer duration-300 text-[2.3vh] focus:ring-0 border-none outline-none w-full sm:w-[94%] pb-2 px-1 pt-2 lg:pb-0 lg:pt-2 bg-transparent text-[#fff]`} onChange={(e)=>{modifierFunction(e.target.value)}}/>
      ...
      <div className={`flex relative items-center border-b-[1.7px] pt-2 pr-1 border-[#fff]`}> 
         <input type={type} placeholder=' ' autoComplete='on' className={`[&:focus_+_label]:!translate-y-[-83%] text-[2.3vh] lg:text-[20px] peer duration-300 [&:focus_+_label]:!text-[12px] focus:ring-0 border-none outline-none w-full px-3 pb-2 pt-2 lg:pb-0 lg:pt-2 bg-transparent text-[#fff] `} onChange={(e)=>{modifierFunction(e.target.value)}}/>
