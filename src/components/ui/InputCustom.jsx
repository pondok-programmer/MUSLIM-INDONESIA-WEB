...
const InputCustom = ({type = 'text',placeholder,id,className,classNameDiv,icon}) => {
  ...
  return (
    <div>
        { type === 'password' ? <><div className={`flex items-center ${classNameDiv}`}>{icon}<input type={isShow ? 'text' : 'password'} placeholder={placeholder} className={`${className} bg-transparent text-[#fff]`} />{isShow ? <BsEyeSlashFill  className='text-2xl md:text-[25px] text-[#fff] mr-2 cursor-pointer' onClick={()=>setIsShow(!isShow)}/> : <BsEyeFill  className='text-2xl md:text-[25px] text-[#fff] mr-2 cursor-pointer' onClick={()=>setIsShow(!isShow)}/>}</div>{showError && <p className='text-red-700'>password not match!</p>}</> 
